...
**Logic — ServiceExtension.cs:** Add one line after the existing `SOAPIntegration` registration:
...
**Agent Instruction:** "Add the single `AddScoped` line in `ServiceExtension.cs` immediately after the `SOAPIntegration` registration. Add the `AppIdToIntegration` entry to `appsettings.json`. No other changes."
...
* [x] **T-01 to T-04:** Payload Validation — `{{CorrelationId}}` injected as unique GUID; template immutability confirmed
* [x] **T-05 to T-10:** Error Handling — `isNegative=true`, HTTP failures, SOAP Fault, missing config
* [x] **T-11 to T-15:** Success Confirmation — positive ACK path for all write operations; REST GET success
* [x] **T-16 to T-19:** Action Mapping — `SOAPAction` header present; correct WSDL endpoint used; correct EML `<action>` tag in body
* [x] **T-20 to T-23:** Conditional Logic — empty identifier on positive ACK; payload userId extraction guard; immutability
* [x] **T-24 to T-28:** REST Integration for GET — HTTP GET method; URL encoding; JSON parsing; V1 vs V2 endpoint selection
...
| `ServiceExtension.cs` | 1 new `AddScoped` line | Low — additive only |

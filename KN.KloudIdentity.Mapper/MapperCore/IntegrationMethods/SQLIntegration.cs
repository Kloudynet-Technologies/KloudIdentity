...
public class SQLIntegration : IIntegrationBase
{
    ...
    public async Task<dynamic> GetAuthenticationAsync(AppConfig config, SCIMDirections direction = SCIMDirections.Outbound, CancellationToken cancellationToken = default)
    {
        ...
        if (config.AuthenticationDetails == null || config.AuthenticationDetails?.Driver == null || config.AuthenticationDetails?.Database == null
            || config.AuthenticationDetails?.Server == null ||config.AuthenticationDetails?.UID == null || config.AuthenticationDetails?.PWD == null)
        {
    ...
    public async Task ProvisionAsync(dynamic payload, AppConfig appConfig, string correlationId, CancellationToken cancellationToken = default)
    {
        ...
        var parameters = payload as List<OdbcParameter> ?? throw new ArgumentNullException("No valid SqlParameter found in the provided payload.");
        ...
        var connection = (await GetAuthenticationAsync(appConfig, SCIMDirections.Outbound, cancellationToken) as OdbcConnection)!;
    ...
    public async Task<Core2EnterpriseUser> GetAsync(string identifier, AppConfig appConfig, string correlationId, CancellationToken cancellationToken = default)
    {
        ...
        var attribute = appConfig.UserAttributeSchemas.FirstOrDefault(a => a.SourceValue == "Identifier" && a.HttpRequestType == HttpRequestTypes.POST)
            ?? throw new ArgumentException("Matching attribute not found for 'identifier'.");

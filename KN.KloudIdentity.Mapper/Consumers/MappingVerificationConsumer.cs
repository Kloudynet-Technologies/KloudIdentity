...
namespace KN.KloudIdentity.Mapper.Consumers
{
    public class MappingVerificationConsumer : IConsumer<MappingVerification>
    {
        ...
        {
            var result = await _getVerifiedAttributeMapping.GetVerifiedAsync(context.Message.AppId,context.Message.correlationId, context.Message.ObjectType, context.Message.HttpMethod);
